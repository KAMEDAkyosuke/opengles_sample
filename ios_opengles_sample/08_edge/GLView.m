...
- (void)render:(CADisplayLink*)displayLink
{
    {// pass0
        ...
        GLKVector4 position;
        GLKMatrix4 result;
        result = GLKMatrix4Multiply(projectionMatrix, camera);
        result = GLKMatrix4Multiply(result, modelrotate);
        result = GLKMatrix4Multiply(result, modelView);
        position = GLKMatrix4MultiplyVector4(result, GLKVector4Make(pass1_vertex[0].position[0],
                                                                    pass1_vertex[0].position[1],
                                                                    pass1_vertex[0].position[2],
                                                                    1.0f));
        static float depth_min = 1.0;
        static float depth_max = -1.0;
        float depth = position.z / position.w;
        depth_min = depth_min < depth ? depth_min : depth;
        depth_max = depth < depth_max ? depth_max : depth;
        NSLog(@"depth = %@, %@, %@", @(depth_min), @(depth_max), @(depth));
    }
    {// pass1
        glBindFramebuffer(GL_FRAMEBUFFER, pass1_buffers.framebuffer);
        glBindRenderbuffer(GL_RENDERBUFFER, pass1_buffers.colorrenderbuffer);
        ...
        glVertexAttribPointer(PASS1_ATTRIB_TEXCOORD, 2, GL_FLOAT, GL_FALSE, sizeof(vertex_t), (GLvoid*) (sizeof(float) * 10));
